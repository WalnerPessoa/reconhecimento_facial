...
def load_encodings(pickle_file):
    with encodings_lock:  # Garantir que o acesso seja controlado ao carregar as codificações.
        with open(pickle_file, 'rb') as f:
            data = pickle.load(f)
    print(f"[INFO ENCODING] Encodings carregados: {data['users']}")  # Verificação de carregamento
    return data['users']
...
def check_for_new_encodings(encodings_file, last_mtime):
    current_mtime = os.path.getmtime(encodings_file)  # Verifica o tempo de modificação do arquivo

    if current_mtime > last_mtime:  # Se o arquivo foi modificado, recarrega-o
        print("[INFO] Atualizando codificações faciais...")
        last_mtime = current_mtime
        return load_encodings(encodings_file)  # Recarrega as codificações
    return None  # Se não houver mudança, retorna None
...
def detect_faces(encodings_file, check_interval=10, resize_scale=0.7, forget_frames=28, model_detection="hog"):
    users = load_encodings(encodings_file)  # Carrega o arquivo no início
    ...
    while True:
        ...
        if current_time - last_check_time >= check_interval:
            new_encodings = check_for_new_encodings(encodings_file, last_mtime)  # Verifica atualizações
            if new_encodings:
                users = new_encodings  # Se houver novas codificações, atualiza
